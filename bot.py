...
# We tell the AI to ONLY look for these characters.
SOLANA_CHARS = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"

print("Loading Neural Network...")
# gpu=False forces CPU mode, which is stable on free servers
...
def preprocess_image_to_memory(input_path):
    ...
    PRODUCTION SETTINGS:
    - No cropping (Safest)
    - 1.5x Scale (Fast but readable)
    - Memory processing (No disk lag)
    """
    with Image.open(input_path) as img:
        ...
        # 1x is too risky for small text. 3x is too slow.
        new_w, new_h = int(w * 1.5), int(h * 1.5)
        img = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
        
        # High Contrast to make text pop against dark backgrounds
...
def batch_check_dex(candidates):
    """
    SPEED ENGINE: Checks 30 tokens in 1 API call
    """
    valid_pairs = []
    unique_candidates = list(set(candidates))
    
    # Chunk into batches of 30 (DexScreener limit)
    chunk_size = 30
...
def generate_mutations(candidate):
    """
    SURGICAL MUTATION: Fixes OCR typos like 'A' vs '4'
    """
    mutations = {candidate}
    ...
    for i, char in enumerate(candidate):
        if char in confusions:
            for replacement in confusions[char]:
                # Swap just this one character to test the theory
...
def fast_mine(text_results):
    ...
    # 1. Extract potential blocks (Whitelisted chars only)
...
def handle_photo(message):
    ...
    try:
        # Process in memory (RAM) for speed
        img_array = preprocess_image_to_memory("scan.jpg")
        
        # Read text with Strict Whitelist
    ...
    if ca and pair:
        ...
        # If it fails, give me the debug text so I can see what happened
